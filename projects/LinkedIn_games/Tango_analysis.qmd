...
In this section, boxplots are created to visualize the difference in scores between PM and CW across the four LinkedIn games.
...
Here, using Bayesian Independent-samples t-tests, evidence for or against a difference in performance between PM and CW for each game type is quantified. For each comparision, the Bayes factor (BF~10~) to measure the relative support for the alternative hypothesis (that the two players differ in average game scores) over the null hypothesis (no difference). 
...
- Peeking and Prior-Data Conflict
...
Proper sequntial analysis would require recalculating the posterior at each stage and using that as the new prior for the next stage. Here, the analysis always begins from the default prior, which may under or overestimate evidence until enough data have accumulated. **Each Bayes Factor value should be regarded as snapshots of a formal sequentally updated analysis.**
...
Runs Levene's test as a check of homogenity of variance for each game type subsets.
